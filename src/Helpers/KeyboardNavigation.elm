module Helpers.KeyboardNavigation exposing (Key(..), onKeyDown, navigateWrappedList, Direction(..), FocusResult, setFocus)
...
type Key =
  RETURN | LEFT | UP | DOWN | RIGHT | DELETE | UNKNOWN
...
navigateWrappedList xs currentX direction =
  let
    ...
    lookup = (\idx -> Dict.get idx idxToX)
    maxIdx = (List.length xs) - 1
    goForward =
...
onKeyDown processKey keys =
  ...
  in
  E.onWithOptions "keydown" options
  decoder
